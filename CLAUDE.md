...
## Monorepo: bitta repo, alohida-alohida deploy

Loyiha bir nechta mustaqil qismdan iborat bo'lsa (masalan `web`, `landing`, `backend`, `bot`), hammasi **bitta GitHub repo**ning ildizida alohida papkalarda turadi — taqdimotga bitta repo linki beriladi, lekin har bir qism platformada **alohida deploy** bo'ladi:
...
- Har bir papka — o'zining **alohida `package.json`**iga ega, mustaqil ishga tushadigan loyiha (bir-biriga import qilinmaydi).
- Deploy qilishda platforma sozlamasida **"Root Directory"** shu papkaga ko'rsatiladi (Render va Vercel ikkalasi ham qo'llab-quvvatlaydi) — bitta repo'dan 3-4ta alohida service/project yaratiladi:
  - `web/` → Vercel project (Root Directory = `web`)
  - `landing/` → Vercel project (Root Directory = `landing`)
  - `backend/` → Render Web Service (Root Directory = `backend`)
  - `bot/` → Render Web Service/Background Worker (Root Directory = `bot`)
- Har bir service GitHub repo'ga ulab qo'yilsa, keyingi `git push`larda hammasi **avtomatik** qayta deploy bo'ladi — qo'lda qayta yuklash shart emas.
- Taqdimotda **bitta repo linki** yetarli — README'da har bir qismning nomi va live URL'i alohida-alohida yozilgan bo'lsin (masalan: Web — link, Landing — link, Backend API — link, Bot — @username).
